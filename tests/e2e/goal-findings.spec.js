...
test('goal search findings deduplicate and retain owner review without rendering provider markup', async ({ page }) => {
  ...
  try {
    ...
    dedicated.handle.toolRegistry.get('web.search').execute = async () => ({ mock: true, results: [
      { title: `<script>bad()</script> Research role ${'x'.repeat(300)}`, snippet: '<img src=x onerror=bad()> Remote opportunity', url: 'https://example.test/role?id=2&utm_source=fixture' },
    ...
    await expect(goals.locator('.goal-research-update')).toContainText('1 new links in indexed evidence · 0 seen before');
    ...
    await expect.poll(() => goals.locator('.goal-evidence').evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBe(true);
    ...
    await expect(goals.locator('.goal-research-update')).toContainText('0 new links in indexed evidence · 1 seen before');
