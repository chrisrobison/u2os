...
test('goal search findings deduplicate and retain owner review without rendering provider markup', async ({ page }) => {
  ...
  try {
    ...
    agent.planner.modelProvider = { id: 'fixture', destination: 'local_model', plan: async () => ({
      reasoning_summary: 'Search roles', actions: [{ tool: 'web.search', arguments: { query: 'remote research roles' } }],
    }) };
