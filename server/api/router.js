...
function rateBucket(m, p) { if (m === 'POST' && p === '/api/auth/login') return 'login'; if (m === 'POST' && p.startsWith('/api/actions/')) return 'actions'; if (m === 'POST' && p.startsWith('/api/agent/')) return 'agent'; if (WRITES.has(m) && p.startsWith('/api/goals')) return 'goals'; if (WRITES.has(m) && p.startsWith('/api/triggers')) return 'triggers'; if (WRITES.has(m) && (p.includes('/credentials') || p === '/api/model')) return 'credentials'; if (WRITES.has(m) && p === '/api/voice/enrollment') return 'enrollment'; return null; }
