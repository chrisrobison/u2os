...
export function registerAuthRoutes(router, { auth, agent }) {
  ...
  router.post('/api/auth/setup', async (req, res) => { const ownerId = await auth.setup(req.body?.passphrase); agent?.setOwnerEntityId(auth.ownerEntity().id); const session = auth.createSession(ownerId); sendJson(res, 201, { authenticated: true, csrfToken: session.csrf }, { 'Set-Cookie': auth.cookie(req, session.token) }); });
