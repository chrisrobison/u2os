...
## Technical debt / known gaps after Phase 7 (all phases complete)

- **No request body size limit anywhere.** `server/api/router.js`'s JSON body parser has no cap, inherited by every route in the app — found during Phase 7's security review while auditing `POST /api/feedback`'s `detail` field specifically, but it's not specific to that route or to this phase; it's a gap in the shared router. Worth a small, generic fix (a max body size the router enforces once, for everyone) rather than a per-route patch.
- **The no-auth gap remains the single biggest piece of real security debt in this codebase**, called out repeatedly across Phase 3 (connector credentials), Phase 4/5 (voice enrollment), and Phase 6 (trigger/recommendation management): every route is equally reachable by anyone who can talk to the server on the network. Nothing built across Phases 1-7 papers over this with inconsistent one-off auth on individual routes — it's tracked as one thing to fix once, with a real auth layer, before this is exposed beyond a trusted local network. This is the most important thing to do before any further phase.
- Email-edit detection (Phase 7) only catches a `draftId` explicitly threaded from `email.draft` to `email.send` within one turn — see docs/feedback.md and `server/feedback/email-edit-detector.js` for the honest scope (fixed during security review from an earlier, unsound correlation-id-only match that could misattribute edits to the wrong draft).
- The mock model provider (Phase 1) still only recognizes a handful of demo intents — real LLM integration (the actual "bring your own AI" model routing described in PROMPT.md §27) is the natural next major piece of work once auth is addressed.
