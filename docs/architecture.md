...
- **Durable action delivery state** is separated from authorization history. `action_queue` stores stable idempotency keys, due times, actor provenance, leases, retry classes, approval/policy references, and terminal delivery state; `action_attempts` retains numbered attempt history. Autonomous and owner-approved actions are queued before `ActionQueueWorker` invokes `ActionExecutor`. The worker re-resolves the registered tool and re-evaluates current policy, approval, and freshness immediately before execution. Queue claims are a single conditional SQLite update, so duplicate scheduler ticks cannot lease the same live item. Expired leases recover after restart; an uncertain external outcome is replayed only when the tool explicitly supports the same durable idempotency key. Otherwise the recovered-attempt guard stops as `outcome_uncertain`, requires review of the originally bound provider/account and denies requeue without recording another attempt. Historical expired attempts remain unchanged; unattempted leases and already-authoritative success do not become uncertain merely because a lease expired. Earlier policy/cancellation/account gates retain their existing attention states. Retries use bounded deterministic backoff, and stale lease owners cannot settle an attempt.
- The owner can inspect sanitized delivery state at `#/operations` via `GET /api/actions/operations`. It shows tool names, lifecycle states, attempt counts, times, failure classes and the persisted original account label/provider/instance, plus associated SMTP sender label/instance/From when present. Identity is allowlisted from the action binding, never resolved from the active selection or credentials; absent/corrupt legacy identity remains unavailable. Arguments, recipient/body, actor identifiers, idempotency keys, credential/revision/transport metadata and raw provider errors remain omitted. Explicit `outcome_uncertain` cards require original-provider review and explain no automatic retry/requeue without claiming failed delivery; restored-snapshot uncertainty remains distinct. Native DOM text rendering and wrapping keep identity inert/readable. Committed queue transitions publish metadata-only `agent.action.queue_updated` events so the view refreshes through the normal SSE path. Waiting entries offer explicit Review approval: only then does the owner-only action endpoint load one original proposal. Exact ID/pending state and external account identity must be present before the existing approval card appears. Its normal approve/reject API retains runtime policy/account checks; no retry or new execution endpoint is added. Single-flight metadata refresh preserves the open card and disabled in-flight controls; closing the preview does not cancel its action. Stale/failed detail fetches show fixed guidance without private server errors. The dashboard's existing approval cards remain available independently; restored conversation turns are textual only.
- Inline approval cards retain the runtime's explicit uncertainty class and show fixed original-account review/no automatic retry guidance, without raw provider errors/results. Chat and voice use the same deterministic outcome sentence: only confirmed executed actions receive scoped completion wording; approved/queued/in-progress/retry-wait delivery stays pending and stopped work points to Operations without claiming no external effect. Contradictory uncertain/pending cards cannot offer approval. This presentation neither authorizes execution nor verifies objective completion.
