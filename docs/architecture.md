...
`goals` currently holds owner-scoped, revisioned objective drafts with completion criteria, constraints, intended scope, and budget caps. It is separate from conversations, runs, and actions. No runtime reads drafts to schedule or execute work yet; the API explicitly reports `executionEnabled: false` and no spent resources. See [Goal drafts](goals.md).
