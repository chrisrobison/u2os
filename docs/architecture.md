...
`conversations` and the pre-existing `conversation_messages` table now hold owner-scoped chat turns separately from runs, structured memory, and proposed memory candidates. Each text or voice request links its new run and user/assistant turns to one explicit conversation ID. A failed request saves a generic system failure turn, never a false successful assistant response. The authenticated API lists only the owner's conversations and a capped recent transcript; the browser stores only the selected ID locally and can start a new chat. The restored transcript is textual only: approval cards are not reconstructed on refresh, while their authoritative state remains in runs/actions. Existing unlinked legacy message rows are preserved, not guessed into an owner conversation. Conversation history is not yet fed to the model or promoted to personal facts; privacy-filtered history retrieval and ambiguous follow-up references remain separate work.
