...
- Editing a drafted email before sending (`email.draft` → user changes the body → `email.send`) is detected server-side by comparing the sent args against the original draft's stored args when both share a `correlation_id`, recorded automatically as `outcome: 'edited'` — no separate UI action needed for this one.
