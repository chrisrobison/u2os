...
| Agent runs | One-pass plan, policy gate, durable action queue; within-plan dependency gate (#191); durable run/step metadata and restart reconciliation (#193) | Node and browser approval/queue suites; dependency status, transitive-skip, run/approval/restart, and old-schema fixtures | No | Model-bound observations, approval continuation, limits, objective verification |
