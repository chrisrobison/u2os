...
| Agent runs | One-pass plan, policy gate, durable action queue; within-plan dependency gate (#191); durable run/step metadata and restart reconciliation (#193); model-bound observation privacy filter (#198) | Node and browser approval/queue suites; dependency status, transitive-skip, run/approval/restart, old-schema, and observation privacy/fallback/injection fixtures | No | Actual multi-step continuation, approval resume, limits, objective verification |
