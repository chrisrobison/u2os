...
| Agent runs | Policy gate and durable action queue; within-plan dependency gate (#191); durable run/step metadata and restart reconciliation (#193); model-bound observation privacy filter (#198); bounded continuation from completed read results with validated references (#194); deferred same-plan dependencies resume after approval (#201) | Node and browser approval/queue suites; dependency status, transitive-skip, run/approval/restart, old-schema, observation privacy/fallback/injection, multi-step draft, and approval-dependent fixtures | No | New model planning after approval/restart (#196), cross-step resource budgets/cancellation, objective verification |
