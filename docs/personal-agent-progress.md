...
| Agent runs | Policy gate and durable action queue; within-plan dependency gate (#191); durable run/step metadata and restart reconciliation (#193); model-bound observation privacy filter (#198); bounded read-result continuation with validated references (#194); deferred same-plan dependencies resume after approval (#201); persisted bounded checkpoints after approval/restart including completed consequential results (#203); durable cancellation between steps and safe unleased-queue revocation (#205) | Node and browser approval/queue suites; dependency status, transitive-skip, run/approval/restart, old-schema, observation privacy/fallback/injection, multi-step draft, approval-dependent, checkpoint, and in-flight cancellation fixtures | No | Automatic event wake after delayed queue completion, cross-step resource budgets/spend, objective verification |
