...
| Agent runs | Policy gate and durable action queue; within-plan dependency gate (#191); durable run/step metadata and restart reconciliation (#193); model-bound observation privacy filter (#198); bounded continuation from completed read results with validated references (#194) | Node and browser approval/queue suites; dependency status, transitive-skip, run/approval/restart, old-schema, observation privacy/fallback/injection, and multi-step draft fixtures | No | Approval/restart continuation, cross-step resource budgets/cancellation, objective verification |
