...
3. Google redirects back to `GET /api/connectors/google/oauth/callback?code=&state=`. Server validates `state` against the cache (reject/expire replay or forged callbacks — this is the CSRF protection for the flow), exchanges `code` at `https://oauth2.googleapis.com/token`, stores `{ access_token, refresh_token, expiry }` under `google.enc.json`'s `tokens.<service>`, marks that service connected, redirects the browser to `/#/connectors?connected=<service>` (or `?error=...`).
