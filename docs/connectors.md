...
- Email search/read tool results preserve `from_addr` and additionally expose `sender_address`, a conservative single source mailbox for recipient references. Common bare ASCII dot-atom mailboxes and display-name angle forms (including one quoted name containing commas) retain exact mailbox bytes. Lists, groups, comments, malformed/control-containing headers, quoted local parts, non-ASCII mailbox addresses and other unsupported forms yield `null`, requiring clarification rather than a guessed recipient. This intentionally narrow subset is not a complete [RFC 5322 parser](https://www.rfc-editor.org/info/rfc5322/). The header remains untrusted: this field is neither sender authentication nor Reply-To selection. Existing destination privacy filtering, exact account binding and send preflight still apply; no cache/schema migration or threaded-reply support is added.
