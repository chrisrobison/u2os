...
The **Automation** screen (`#/automation`) lists every system and user trigger with its kind, source, current state, schedule summary, and next check where applicable. Owners can pause or resume any trigger, expand recent-run history, and request a manual dry run. `GET /api/triggers/:id/history?limit=` derives bounded history from the immutable event log and exposes only run id/time, completed-or-failed status, event/action kind, and correlation id; action results, arguments, and raw errors are deliberately omitted. `POST /api/triggers/:id/dry-run` is a read-only preview: timers/schedules report whether they are currently due, event rules report the event they await, and built-in condition watches report only a current match count. It never publishes an event, invokes a tool/model, advances a schedule, or consumes dedupe state. The constrained creation form supports one-time timers and recurring minute schedules; it sends only structured `kind` and `config` data to the existing validated trigger API. User-created triggers can also be deleted after confirmation. Seeded system triggers deliberately have no delete control so the built-in proactive behaviors remain recoverable, although they can be paused.

Event-rule and condition-watch authoring remain API-only because their richer action and matching configuration needs a purpose-built safe editor. Pause/resume, persisted next-run display, bounded execution history, and side-effect-free manual dry runs are implemented.
