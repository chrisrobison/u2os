...
Wired to fire on the event types PROMPT.md explicitly lists as proactive-worthy: `email.received`, `calendar.event_approaching`, `task.overdue`, `calendar.event_changed` (conflict detection), `project.changed`... — the current implementation covers `email.received` (recruiter-sender heuristic → `notify`), `calendar.event_approaching` (→ `prepare`: generates a "before-meeting" dashboard via the Phase 2 dynamic-dashboard generator and attaches it to a recommendation card), `calendar.event_changed` (deterministic overlap detection against the authoritative local mirror → `notify`), `task.overdue` (→ `notify`), and `commitment.made` (→ `act`: auto-creates the linked task, tying together with `memory/projector.js`'s existing commitment detection). Project activity, renewals, and broader important-message evaluation remain unwired and must not be represented as implemented.
