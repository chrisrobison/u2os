...
Use `npm run backup -- --encrypt /private/backup-location/u2os.tar.gz.enc` for authenticated encryption with an independent masked passphrase (or an explicitly supplied noninteractive secret environment variable). Encrypted restore authenticates the complete archive privately before entry validation, and `--encrypt` requires encrypted input without plaintext fallback. See [format, secret handling and offline verification guidance](backups.md). Default legacy `.tar.gz` creation remains explicitly unencrypted and includes the credential master key. Restore publishes only validated regular files/directories into an empty isolated home under ownership; force-merging is refused. Restored homes remain inactive, with runtime and mutation CLIs blocked before migrations/reconciliation. Explicit activation/original-instance retirement are unfinished: never remove the recovery marker or run original/restored copies concurrently. Authenticated `GET /api/export` produces portable JSON without connector secrets. U2OS has no built-in TLS termination, supported public-internet recipe, Windows service, distributed limiter, or production rollback system.
