...
Optional: choose **Schedule one read-only pass** to persist one future wake. The server must be running; restart picks up a pending overdue wake on a scheduler tick. Inspect its linked run/blocker before scheduling again. A [finite web research schedule API](goals.md#finite-scheduled-web-research) supports 2–10 daily-or-slower passes; native creation controls are still a follow-up. It stops on unsuccessful/uncertain work and never catches up missed intervals in a burst. Browser closure does not cancel an already-started bounded run; inspect persisted evidence after returning. Failed/uncertain prerequisites are not treated as successful findings.
