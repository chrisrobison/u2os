...
Optional: choose **Schedule one read-only pass** to persist one future wake. The server must be running; restart picks up a pending overdue wake on a scheduler tick. Inspect its linked run/blocker before scheduling again. There is no recurring schedule yet. Browser closure does not cancel an already-started bounded run; inspect persisted evidence after returning. Failed/uncertain prerequisites are not treated as successful findings.
